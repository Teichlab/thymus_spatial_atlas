...
def bin_axis_new(ct_order, cutoff_values, df, axis_anno_name):
    ...
    Returns:
        pandas.DataFrame: The modified DataFrame with manual bin labels assigned.
    """
    # Define the column name for manual bins
    category_column_name = 'manual_bin_' + axis_anno_name
    
    # Remove the old manual bin column if it exists
    if category_column_name in df.columns:
        df = df.drop(columns=[category_column_name])
    
    # Initialize the column with 'unassigned' category
    df[category_column_name] = pd.Categorical(['unassigned'] * len(df), categories=['unassigned'] + ct_order)
    
    # Assign values based on the cutoff values
    df.loc[df[axis_anno_name] < cutoff_values[0], category_column_name] = ct_order[0]
    print(f"{ct_order[0]} = ({cutoff_values[0]} > {axis_anno_name})")
    
    for idx in range(len(cutoff_values) - 1):
        df.loc[(df[axis_anno_name] >= cutoff_values[idx]) & (df[axis_anno_name] < cutoff_values[idx + 1]),
               category_column_name] = ct_order[idx + 1]
        print(f"{ct_order[idx + 1]} = ({cutoff_values[idx]} <= {axis_anno_name}) & ({cutoff_values[idx + 1]} > {axis_anno_name})")

    df.loc[df[axis_anno_name] >= cutoff_values[-1], category_column_name] = ct_order[-1]
    print(f"{ct_order[-1]} = ({cutoff_values[-1]} <= {axis_anno_name})")

    # Remove unused categories
    df[category_column_name] = df[category_column_name].cat.remove_unused_categories()

    # Convert to categorical and ensure it's treated as such in the DataFrame
    df[category_column_name] = df[category_column_name].astype('category')
    df[category_column_name + '_int'] = df[category_column_name].cat.codes
...
def plot_random_color(img, save_path=None):
    """
    Assigns a random color to each unique label in the input image and plots the result.

    For 3D images, it plots each slice (along the first dimension) as separate subplots. 
    For 2D images, it simply plots the colored image.
    If a save path is provided, the resulting plot is saved to the given location with the original pixel dimensions.
    ...
    Notes
    -----
    This function requires matplotlib's pyplot (plt) module and numpy.
    """
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    # Calculate the figure size in inches and DPI to match the original image resolution
    image_height, image_width = img.shape[-2], img.shape[-1]
    dpi = 100  # Standard DPI, adjust as needed
    figsize = (image_width / dpi, image_height / dpi)

    if len(img.shape) == 3:
        # Calculate number of rows and columns for subplots
        num_rows = colored_img.shape[0] // 2 + 1
        num_cols = 2

        # Adjust figure size for multiple subplots
        figsize = (image_width / dpi * num_cols, image_height / dpi * num_rows)
        ...
        ax.axis('off')
        ax.set_aspect('equal', adjustable='box')  # adjust the aspect to equal
...
def process_and_plot_gam_model(adata, gene, obs_columns, threshold, spline_params,jitter=False,report=True):
    """
    Process an AnnData object and fit a Generalized Additive Model (GAM) for predictive modeling.
    The function extracts gene expression data as the target variable and uses other observations as features for the model.
    It also plots the model and its output.

    Parameters
    ----------
    adata : anndata.AnnData
        An AnnData object containing gene expression data and observations.
    gene : str
        The gene name whose expression is to be predicted.
    obs_columns : List[str]
        A list of observation column names to extract and process from the AnnData object's obs DataFrame.
    threshold : float
        Cutoff for gene expression; genes with expression below this threshold are excluded.
    spline_params : dict
        A dictionary containing spline parameters for numeric features (e.g., {'n_splines': 9, 'spline_order': 2, 'lam': 0.5}).
    jitter : boolean 
        if to plot the scatterplot 
    report : boolean
        if the show the model summary or plot (default : True)

    Returns
    -------
    LinearGAM
        A fitted Generalized Additive Model (GAM).
    """
    from functools import reduce
    from pygam import LinearGAM, s, f
    from sklearn.preprocessing import LabelEncoder

    # Filter the AnnData object based on gene expression threshold
    adata_filtered = adata[adata.obs_vector(gene) > threshold].copy()

    # Prepare the response variable (target): gene expression
    y_df = pd.DataFrame(adata_filtered.obs_vector(gene), index=adata_filtered.obs.index, columns=[f'{gene}_y'])

    # Process observation data (features)
    obs_data = adata_filtered.obs[obs_columns].copy()
    obs_data_encoded = obs_data.copy()

    # Encode categorical columns and leave numeric columns unchanged
    for column in obs_columns:
        if obs_data[column].dtype.name == 'category' or obs_data[column].dtype == object:
            label_encoder = LabelEncoder()
            obs_data_encoded[column] = label_encoder.fit_transform(obs_data[column])
        # No need to do anything for numeric columns - they are already numeric

    # Combine with the target variable, aligning indices
    df_encoded = pd.concat([obs_data_encoded, y_df], axis=1)

    # Dynamically build the terms for the GAM model based on the data type
    terms = []
    for i, col in enumerate(df_encoded.columns[:-1]):  # Exclude the target column
        if 'int' in str(obs_data[col].dtype) or 'float' in str(obs_data[col].dtype):
            # For numeric features, use a spline term
            spline_terms = s(i, **spline_params)
            terms.append(spline_terms)
        else:
            # For categorical or integer-encoded features, use a factor term
            factor_terms = f(i)
            terms.append(factor_terms)

    # Combine all the terms using the '+' operator to create a TermList
    gam = LinearGAM(reduce(lambda a, b: a + b, terms)).fit(df_encoded.drop(columns=[f'{gene}_y']), df_encoded[f'{gene}_y'])

    if report:
        # Print the model summary
        gam.summary()

        sns.set_theme(style="whitegrid")  # Set the Seaborn theme if you need

        # Calculate the mean of the response variable
        response_mean = df_encoded[f'{gene}_y'].mean()

        for i, term in enumerate(gam.terms):
            if term.isintercept:
                continue

            # Generate the grid for the term
            XX = gam.generate_X_grid(term=i)
            # Calculate the partial dependence
            pdep, confi = gam.partial_dependence(term=i, X=XX, width=0.95)

            # Offset the partial dependence plot by adding the mean of the response variable
            pdep += response_mean
            confi = np.array(confi) + response_mean  # Assuming confi is a tuple of numpy arrays

            # Plotting
            plt.figure()
            plt.plot(XX[:, term.feature], pdep, label='Spline')
            plt.plot(XX[:, term.feature], confi, c='r', ls='--')
            if jitter:
                jitter_strength = 0.05  # Adjust this value as needed
                jitter = np.random.uniform(-jitter_strength, jitter_strength, df_encoded.iloc[:, term.feature].shape)
                plt.scatter(df_encoded.iloc[:, term.feature] + jitter, df_encoded[f'{gene}_y'], alpha=0.5, s=0.5, label='Training Data')

            plt.title(obs_data.columns[i])
            plt.legend()
            plt.show()

    return gam

def cumulative_pca(adata,obs,ncomps=10):
    ...
    obs_name1 = obs  # replace with your observation name

    # Get the values of the observation
    obs_values1 = adata.obs[obs_name1]
    ...
    for i in range(ncomps+1):
        ...
        explained_variance = adata.uns['pca']['variance_ratio'][i]
